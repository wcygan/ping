...
```bash
kubectl get kafka -n kafka-system ping-kafka-cluster -o yaml
apiVersion: kafka.strimzi.io/v1beta2
kind: Kafka
metadata:
  annotations:
    kubectl.kubernetes.io/last-applied-configuration: |
      {"apiVersion":"kafka.strimzi.io/v1beta2","kind":"Kafka","metadata":{"annotations":{"strimzi.io/kraft":"enabled","strimzi.io/node-pools":"enabled"},"labels":{"skaffold.dev/run-id":"a67a30e4-2183-4c77-b502-5efc99e008ad"},"name":"ping-kafka-cluster","namespace":"kafka-system"},"spec":{"kafka":{"config":{"default.replication.factor":3,"inter.broker.protocol.version":"3.7","metadata.version":"3.7","min.insync.replicas":2,"offsets.topic.replication.factor":3,"transaction.state.log.min.isr":2,"transaction.state.log.replication.factor":3},"listeners":[{"name":"plain","port":9092,"tls":false,"type":"internal"},{"name":"external","port":9093,"tls":false,"type":"nodeport"}],"version":"3.7.0"}}}
    strimzi.io/kraft: enabled
    strimzi.io/node-pools: enabled
  creationTimestamp: "2024-10-26T19:06:16Z"
  generation: 1
  labels:
    skaffold.dev/run-id: a67a30e4-2183-4c77-b502-5efc99e008ad
  name: ping-kafka-cluster
  namespace: kafka-system
  resourceVersion: "295853"
  uid: de7f66fa-e29b-43bb-8fd6-57ff8706c528
spec:
  kafka:
    config:
    ...
    version: 3.7.0
status:
  clusterId: 6lu7LEq_R529-X6H8CAgJQ
  conditions:
  - lastTransitionTime: "2024-10-26T19:06:18.876895033Z"
    ...
    status: "True"
    type: Warning
  - lastTransitionTime: "2024-10-26T19:07:02.168044248Z"
    status: "True"
    type: Ready
  ...
  - addresses:
    - host: ping-kafka-cluster-kafka-bootstrap.kafka-system.svc
      port: 9092
    bootstrapServers: ping-kafka-cluster-kafka-bootstrap.kafka-system.svc:9092
    name: plain
  - addresses:
    - host: 192.168.49.2
      port: 30109
    bootstrapServers: 192.168.49.2:30109
    name: external
...
```bash
kubectl -n kafka-system exec -it ping-kafka-cluster-ping-kafka-cluster-pool-0 -- bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --topic test --partitions 3 --replication-factor 3
```

Start a console producer

```bash
kubectl -n kafka-system exec -it ping-kafka-cluster-ping-kafka-cluster-pool-0 -- bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic test
```

In another terminal, start a console consumer

```bash
kubectl -n kafka-system exec -it ping-kafka-cluster-ping-kafka-cluster-pool-0 -- bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic test --from-beginning
```

## Example of creating a topic, producing, and consuming

Setup the topic:

```bash
wcygan@foobar ping % kubectl -n kafka-system exec -it ping-kafka-cluster-ping-kafka-cluster-pool-0 -- bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --topic test --partitions 3 --replication-factor 3
...
```bash
wcygan@foobar ping % kubectl -n kafka-system exec -it ping-kafka-cluster-ping-kafka-cluster-pool-0 -- bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic test
Defaulted container "kafka" out of: kafka, kafka-init (init)
>he llo
>yo yo dawg
```
